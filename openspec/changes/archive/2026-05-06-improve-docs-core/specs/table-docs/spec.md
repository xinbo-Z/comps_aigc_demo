...
文档系统 SHALL 为 Table 提供独立的增强型组件文档页，并以统一骨架组织中文定义、适用场景、结构化案例、常用属性 API 表、源码说明与代码展开/复制能力；页面 SHALL 同时补足受控列配置、组合能力建议以及基础 Table 与高阶业务表格的边界说明。
...
#### Scenario: Table 页面说明受控列能力

- **WHEN** 用户浏览列宽拖拽或列配置相关案例
- **THEN** 页面 SHALL 说明列宽变更结果需要由外部接收、持久化并回写，而不是暗示组件内部自动管理全部列配置状态
...
#### Scenario: Table 页面说明基础 Table 与高阶业务表格边界

- **WHEN** 用户评估复杂查询编排、业务操作联动、重数据流控制或强业务状态同步场景
- **THEN** 页面 SHALL 明确基础 Table 主要负责通用列表承载，而更高阶业务表格能力应由外层业务容器或独立高级组件承担

#### Scenario: Table 页面提供组合能力建议

- **WHEN** 用户需要组合 `scroll`、`virtualScroll`、`rowSelection`、`columnResize` 或同等级能力
- **THEN** 页面 SHALL 提供最小必要的组合建议与边界提示，帮助用户理解这些能力适合解决的问题范围
